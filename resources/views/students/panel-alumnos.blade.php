    ...
    <div class="min-h-screen bg-gray-100 p-8">
        <div class="max-w-4xl mx-auto grid gap-6 grid-cols-1 md:grid-cols-2">
            @forelse ($subjectsMaterials as $index => $subject)
                <!-- Tarjeta de Materia -->
                <div>
                    <button onclick="openModal({{ $index }})" class="w-full bg-white rounded-2xl shadow-md border p-6 flex flex-col items-start hover:shadow-lg transition">
                        ...
                        @if ($firstImage)
                            <img src="{{ asset($firstImage->path) }}" alt="Imagen de {{ $subject->subject_name }}" class="w-full h-40 object-cover rounded-xl mb-4">
                        @else
                            <div class="w-full h-40 bg-gray-200 flex items-center justify-center rounded-xl mb-4 text-gray-400">
                                Sin imagen
                            </div>
                        @endif
                ...
                <div id="modal-{{ $index }}" class="fixed inset-0 bg-black bg-opacity-50 hidden justify-center items-center z-50">
                    <div class="bg-white rounded-2xl p-8 max-w-md w-full relative">
                        <button onclick="closeModal({{ $index }})" class="absolute top-2 right-2 text-gray-400 hover:text-gray-600">
                            ✖
                        </button>
                        ...
                        <div class="space-y-4 max-h-80 overflow-y-auto">
                            @forelse ($subject->materials as $material)
                                <div class="p-4 border rounded-lg bg-gray-50">
                                    ...
                                    <a href="{{ route('students.task.create',$material) }}" class="mt-2 inline-block text-sm font-medium text-blue-600 hover:underline">
                                        Ver actividad
                                    </a>
                                </div>
                            @empty
